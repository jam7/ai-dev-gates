...
- **直した ID を前提にしている下流も読み直す** (D-02 を直したら、同じ
  design.md の D-03 以降を通しで)。**手順 3 の grep では足りない** — 下流は
  上流を名指しせずに前提だけ受け継ぐので、ID は本文に現れない。実例では
  旧前提の上に立つ下流が「書いてあるとおり作ると壊れる」状態で 1 日残り、
  その ID が grep に当たるようになったのは**訂正を書き込んだ後**だった
  (= grep で見つかるのは直し終わったものだけ)。矛盾があれば手順 3 の
  影響表に載せる。ここで気づかないと、影響表には最初から載らない
