...
`gate.conf` の `dup_corpus` で広げられ、corpus が 400 ファイルを超えると
**測らずに「測らなかった」と言います** (`dup_corpus_max`)。リポジトリ全体の
重複を見たいときは cq-review を使います。
...
dup_corpus_max=""                    # corpus がこれを超えたら測らない (空 = 400)
