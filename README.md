...
トレース ID の説明など)。**既にある CLAUDE.md は触りません。**Skill も CLAUDE.md も Markdown なので、チームの規約に
合わせた変更をコミットで共有できます。
...
衝突と数えず「up to date」で済み、**差分があるものだけ**をファイル単位で列挙して
何もせず終了します。手を入れたパッケージのファイルを、再実行で黙って
失うことがないようにするためです。差分 = 旧版なら `--force` で更新します。

### `--force` の意味は 1 つだけです

`--force` は**「パッケージ自身のファイルを新版に置き換えてよい」という同意**です。
それ以外の意味は持ちません。次のファイルは `--force` を付けても触りません。
**そもそも比較対象に入っていません** — こちら側から再生成できない、あなたの
判断そのものだからです。

| ファイル | 扱い |
|---|---|
| `CLAUDE.md` | 無ければテンプレートから作る。**あれば触らない** |
| `coding-rules/rules/*.md` | チームのルール (原本の `*.template.md` だけ更新) |
| `tools/gate.conf` と宣言ファイル (`cq-baseline.txt` / `test-vocabulary.txt` / `private-allow.txt` / `refs-allow.txt`) | 無ければ作る。以後触らない (第 9 章) |
| `settings.json` | マージのみ (第 9 章) |
| `.claude/hooks/` にある他人の hook | 見ない |

**この設計に手を入れる人へ**: `--force` に「利用者のファイルも初期化する」意味を
足したくなったら、それはファイルの所有者分類が間違っている合図です。分類表を
直す方が正しい — フラグを増やすと、日常的に必要な「更新」を選ぶたびに破壊が
付いてきます。**判定側 (衝突検出) だけを変えるのが特に危険です。**書き込み側は
所有者を知っているので、テストでは「保持されている」ように見え、列挙と
`--force` の側だけが壊れます。実際にそうなりました — プロジェクト固有の 123 行の
CLAUDE.md が 28 行のテンプレートに置き換わり、表示は成功に見えていました。
経緯と決定は `docs/install/adr-001-file-ownership.md`、不変条件は
`docs/install/design.md` の D-02 にあります。
...
**解決**: 閾値ではなく**宣言**をゲートにします。指摘は `tools/cq-baseline.txt` に
理由付きで 1 行書かれていなければコミットが通りません。**その 1 行を書くことが
レビュー**です。

**測るのは変更だけです。** staged したファイルを HEAD 版と比べ、**この変更が
増やしたキーだけ**を止めます。既にあった指摘は「触っていないもの」として 1 度
表示するだけで、コミットは通ります。だから**既存のコードベースに今日入れられます** —
返済を待つ必要はありません (`docs/gates/design.md` D-11、ADR-002)。

実測: `llvm/lib/Target/RISCV` は 99 ファイルで指摘 428 件あります。従来の
「リポジトリ全体を測る」方式では、この 428 件に理由を書き終えるまで 1 コミットも
通りませんでした。今は同じディレクトリの 27,000 行のファイルを触っても、
増分が無ければ 0.7 秒で通ります。
...
Already there before this change (nothing to answer for):
  src/legacy.go:1  Legacy()  312 lines

This change adds structural findings that are not declared:
...
上の `Legacy()` は**止めません**。触っていない (触っても悪化させていない) ので、
この変更の責任ではないからです。
...
**baseline の掃除は別コマンドです。** ゲートは変更しか測らないので、宣言は
一度書かれると二度と再検査されません。`check-metrics.py --stale` が
「もう検出されない宣言」を報告します (元プロジェクトでは 5 件中 3 件が
陳腐化していました)。時々流してください。

**重複 (dup) だけは比較なので、何と比べるかを決める必要があります。** 既定は
**触ったファイルと同じディレクトリ**で、近傍からのコピペを捕まえます。
`gate.conf` の `dup_corpus` で広げられます。リポジトリ全体の重複を見たいときは
cq-review を使います。

**時間の上限は `max_files` 1 つです** (既定 400)。実測で約 6.5ms/ファイル、
両版を測るので 400 ファイルで約 3 秒。上限は**2 段階で譲り、譲るたびに言います**。

| 状況 | 挙動 |
|---|---|
| staged したファイル数 > 上限 | 構造チェック全体を飛ばし、飛ばしたと言う |
| corpus (近傍込み) > 上限 | 近傍を捨てて dup を止め、staged だけ測る |

巨大コミット (初回インポート・一括整形) は**ゲートではなく人間のレビュー対象**
という判断です。黙って数分止まるのが一番悪い、という考え方に揃えています。
...
always_checks=""                     # 同上。ただし毎コミット走る (任意)
dup_corpus=""                        # 重複を探す範囲 (空 = 触ったファイルの同階層)
max_files=""                         # 1 回に読むファイル数の上限 (空 = 400)
...
`always_checks` は同じですが、**何を触ったコミットでも走ります**。ドキュメントだけの
コミットで壊れる不変条件はこちらに置きます — `extra_checks` はそのコミットを
見ないためです。実例: このリポジトリでテンプレートだけを直して実体を忘れたとき、
`.py` が staged に無かったのでゲートは 1 度も走りませんでした
(`docs/install/design.md` の D-05)。毎コミット走るので、速いものだけ置いてください。

### いつ入れるか — いつでも入れられます

**変更しか測らないので、既存のコードベースでも導入コストは 0 です。**
`cq-baseline.txt` は空のまま始められます。

以前この章には「入れるのは新規プロジェクトか、返済が打ち止めになったあとだけ」と
書いてありました。ゲートがリポジトリ全体を測っていたころの話で、当時は初回から
大量に落ちるのが避けられませんでした。**その制約は無くなりました** (2026-08-29、
ADR-002)。

ただし、当時の警告のうち 1 つは今も生きています。**`check-metrics.py --list` の
結果をそのまま `cq-baseline.txt` に流し込まないでください。** 宣言の値打ちは
「1 行書くことがレビューになる」点にあるので、理由を書けない行が並んだ時点で
ただの無視リストです。新方式では**そもそも流し込む必要がありません** —
既存の指摘は触るまで出てこないからです。

| 状況 | 使うもの |
|---|---|
| 新規プロジェクト | 最初からゲートを入れる |
| 途中まで進んでいる | **ゲートを入れてよい。** 既存分は触るまで出ない。返済は第 10 章で並行して進める |
| 全体像が知りたくなったら | cq-review (第 4 章) と `check-metrics.py --list` |
...
| Edit / Write の直後 | 同上 | 触ったファイルのパスを記録する |
| ターン終了時 | **記録したファイルを実測**し、メモを見る | 増分があるのにメモが答えていなければ**終了をブロック**する |

3 つめが本体です。ターン終了時に 2 つのことをします。

1. **記録したファイルを `check-metrics.py --paths` で測る** — コミットゲートと
   **同じ判定** (HEAD 比の増分) なので、ここを通ったものはコミットでも通ります。
   既にあった指摘は「触っていないもの」として見せるだけで、答える必要はありません
2. 会話ログ (`~/.claude/projects/<cwd>/<session>.jsonl`) の**そのターン以降の
   発言だけ**を読み、準拠メモがあるか、**増分のキーがメモに書かれているか**を見ます

以前はここが**自己申告でした** — hook は何も測っておらず、`準拠メモ` という
文字列があれば通っていました。今は増分のキー (`long lib/a.dart::parse` の形) を
メモに書くまで止まります。そのキーはそのまま `cq-baseline.txt` に貼れます。

範囲をそのターンに限るのは、過去のターンで書いたメモが今回の欠落を隠さないため
です。ログが読めなかったときも止めます — 空振りの催促はコスト 1 文、見逃しは
レビュー 1 回分だからです。**プロジェクトに `tools/check-metrics.py` が無ければ
測定は飛ばし**、準拠メモの要求だけが残ります (この入口は便利さであって、
保証はコミットゲートが持ちます)。
...
### ステップ 6: ゲートは最初から入れておく

以前ここには「返済が打ち止めになってから、そこで初めてゲートを入れる」と書いて
ありました。**順番が逆になりました** (2026-08-29、ADR-002)。ゲートは変更しか
測らないので、**ステップ 1 で入れて構いません**。

```bash
./install.sh . --hooks
```

`tools/cq-baseline.txt` は**空のまま**始めます。既存の指摘は触るまで出てこないので、
流し込むものがありません。返済バッチ (ステップ 4) で触ったファイルについては、
その場で「直す」か「理由を 1 行書く」かの二択になります — 返済しながら宣言が
育つ形です。

`--list` の出力を機械的に流し込まないでください。それをやると宣言ファイルが
無視リストになり、この仕組みは死にます。
